...
public class EasyBlur {
     ...
     * default scale
     */
    private static final float SCALE = 1 / 8.0F;
     ...
     * 默认使用rs 模糊图片
     */
    private static final int RS_BLUR = 0, FAST_BLUR = 1;
    private @BlurPolicy int mPolicy = RS_BLUR;
    @Retention(RetentionPolicy.SOURCE)
    @Target({ElementType.PARAMETER, ElementType.FIELD})
    @IntDef({RS_BLUR, FAST_BLUR})
    private @interface BlurPolicy {
    }
    /**原来采用枚举实现，现在通过注解实现
    ...
    public Bitmap blur(){
        ...
        // support library running on 2.3(API 9)  and higher 算法
        if(Build.VERSION.SDK_INT > CURRENT_VERSION){
            if(mPolicy == FAST_BLUR){
     ...
     * @return
     */
    public EasyBlur policy(@BlurPolicy int policy){
        this.mPolicy = policy;
        return this;
    }
