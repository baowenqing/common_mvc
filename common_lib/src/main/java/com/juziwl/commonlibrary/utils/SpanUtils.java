...
public final class SpanUtils {
     ...
     * @return
     */
    public static SpannableStringBuilder getColorSpan(int color,String content, int start, int end) {
        SpannableStringBuilder msp = new SpannableStringBuilder(content);
        msp.setSpan(new ForegroundColorSpan(color),
                start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        msp.setSpan(null,start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        return msp;
    }
    ...
    class CustomIconMarginSpan implements LeadingMarginSpan, android.text.style.LineHeightSpan {
        ...
        private Bitmap uri2Bitmap(final Uri uri) {
            try {
                return MediaStore.Images.Media.getBitmap(Global.application.getContentResolver(), uri);
            } catch (IOException e) {
                e.printStackTrace();
        ...
        private Bitmap resource2Bitmap(final int resourceId) {
            Drawable drawable = ContextCompat.getDrawable(Global.application, resourceId);
    ...
    public class CustomImageSpan extends CustomDynamicDrawableSpan {
        ...
        private CustomImageSpan(final Bitmap b, final int verticalAlignment) {
            ...
            mDrawable = new BitmapDrawable(Global.application.getResources(), b);
        ...
        public Drawable getDrawable() {
            ...
            if (mDrawable != null) {
                ...
                Bitmap bitmap;
                try {
                    InputStream is = Global.application.getContentResolver().openInputStream(mContentUri);
                    bitmap = BitmapFactory.decodeStream(is);
                    drawable = new BitmapDrawable(Global.application.getResources(), bitmap);
                    drawable.setBounds(0, 0, drawable.getIntrinsicWidth(), drawable.getIntrinsicHeight());
                    if (is != null) {
                        is.close();
                    }
                } catch (Exception e) {
                    Log.e("sms", "Failed to loaded data " + mContentUri, e);
                }
            } else {
                try {
                    drawable = ContextCompat.getDrawable(Global.application, mResourceId);
                    drawable.setBounds(0, 0, drawable.getIntrinsicWidth(), drawable.getIntrinsicHeight());
                } catch (Exception e) {
    ...
    public SpannableStringBuilder getCustomImageSpan(@DrawableRes final int resourceId) {
        SpannableStringBuilder mBuilder = new SpannableStringBuilder(REPLACE_STR);
        mBuilder.setSpan(new ImageSpan(Global.application, resourceId, DynamicDrawableSpan.ALIGN_BASELINE), 0, REPLACE_STR.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        return mBuilder;
    }
}
