...
public class StringUtils {
    ...
    public static String replaceInputLabel(String content) {
        builder.delete(0, builder.length());
        if (content.contains("<input")) {
            builder.append(content);
            int start = builder.indexOf("<input");
            int end = builder.indexOf(">", start);
            builder.delete(start, end + 1);
            builder.insert(start, "[space]");
     ...
     * @return 00:33
     */
    public static String formatTime(long voiceSecond) {
        long minute = voiceSecond / 60;
        long second = voiceSecond % 60;
        if (minute > 10) {
            if (second > 10) {
                ...
                return String.format(Locale.getDefault(), "%d:0%s", minute, second);
            }
        } else {
            if (second > 10) {
                return String.format(Locale.getDefault(), "0%d:%s", minute, second);
            } else {
                return String.format(Locale.getDefault(), "0%d:0%s", minute, second);
            }
        }
    }
    ...
    public static boolean isPictureUrl(String url) {
        ...
        //先判断URL是否包含width和height字段
        String newUrl;
        if (url.contains("width=") && url.contains("height=")) {
            int indexOf = url.indexOf("\" width");
            newUrl = url.substring(0, indexOf);
        } else {
            newUrl = url;
        }
        ...
        return "JPG".equals(type) || "JPEG".equals(type) || "GIF".equals(type)
                || "PNG".equals(type) || "BMP".equals(type);
    }
     ...
     * number超过1000时，显示999+；
     */
    public static String showNumberOut(int number) {
